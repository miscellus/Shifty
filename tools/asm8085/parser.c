...
static const char *path_resolve(const char *name)
{
    ...
    return _fullpath(resolved_path, name, sizeof(resolved_path));
#else
...
#define PARSE_ERROR "%s(%d): parse error: "
...
struct line *read_file(const char *filename) {
    ...
    const char *file_path = path_resolve(filename);
    ...
    while (!feof(file)) {
        ...
        // Parse the line
        prev = cur; 
        cur = parse_line(line_buf, prev, file_path, &error, &parse_first);
        if (cur == NULL) {
